...
########
# format BIBI data for stacked area plot generated by Tableau Software
# Used to produce charts found at:
# ???
library( LFPStreamKeepersData )
options(width=100)
source(system.file("loadBIBI.R", package="LFPStreamKeepersData"))
names(bibiScoringData$y2006)

siteYear <- function( site, year ) {
    df1 <- eval(parse(text=paste(sep="","bibiScoringData$y", year, "$", site, "[bibiScoringData$y", year, "$", site, "$Replicate=='Composite',c(1,3)]" ) ))
    df1["Year"] <- year
    df1
}

for (site in c( "lyon35th", "lyon178th", "mcAleerAcres", "mcAleerPerkins")) {

    df = data.frame("Metric"=character(),"Year"=numeric(),"Score"=numeric())
    df = rbind( df, siteYear( site, 2006 ))
    df = rbind( df, siteYear( site, 2007 ))
    df = rbind( df, siteYear( site, 2008 ))
    df = rbind( df, siteYear( site, 2009 ))
    df = rbind( df, siteYear( site, 2010 ))
    df = rbind( df, siteYear( site, 2012 ))

    df[,c("Score")] = sapply( df[,c("Score")],as.numeric)
    df = df[c("Metric","Year","Score")]

    write.table(df,file=paste(sep="",site,".txt"),sep="\t",row.names=FALSE)
#write.table(df,file="mcAleerAcres.txt",sep= "\t",row.names=FALSE)
}
